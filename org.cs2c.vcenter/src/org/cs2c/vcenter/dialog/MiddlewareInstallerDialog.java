...
public class MiddlewareInstallerDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		GridData gd_text_4 = new GridData(SWT.FILL, SWT.FILL, false, false, 2, 1);
		...
		btnNewButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent e) {
				...
				text_5.setText(dialog.getFilterPath()+File.separator+dialog.getFileName());
		...
		/*
		text.setText("10.1.50.4");
		text_1.setText("root");
		text_2.setText("cs2csolutions");
		text_3.setText("Nginxq");
		text_4.setText("/root/nginx/");
		*/
		Composite composite = new Composite(container, SWT.NONE);
		GridData gd_composite = new GridData(SWT.FILL, SWT.CENTER, false, false, 3, 1);
		...
		btnInstall.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent e) {
				...
				if(contentIsValid())
				{
					...
					hostInfo.setMiddleware("Nginxq");
					hostInfo.setPassWord("cs2csolutions");
					hostInfo.setUserName("root");
					*/
					
					if(hostXml.hasHostInfo(hostInfo))
					{
						MessageBox mmb=new MessageBox(getShell(), SWT.OK);
						mmb.setMessage("The unique identification has been used, please refill the middleware's name !");
						mmb.setText("Warning");
						mmb.open();
					}
					else
					{
						...
						if(hostXml.insertHostInfo("conf/host.xml", hostInfo))
						{
							...
							mb.setMessage("Install successfully and the host information has been written to host.xml!");
							mb.open();
						}
						else
						{
							MessageBox mb=new MessageBox(getShell(),SWT.OK);
							mb.setMessage("Install successfully but the host information has not been written to host.xml, please add the host information manually!");
							mb.open();
						}
						setReturnCode(OK);
						close();
...
private boolean contentIsValid() {
		...
		{
			MessageDialog.openWarning(getParentShell(), "Warning", "The install file is empty");
			return false;
		}
		...
		{
			MessageDialog.openWarning(getParentShell(), "Warning", "Install file is invalid");
			return false;
		}
